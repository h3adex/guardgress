...
func Request() (*http2.Request, error) {
...
func RequestFromMap(params map[string]string) (*http2.Request, error) {
	r := new(http2.Request)
	...
	var ok bool
	r.ProtoMajor, r.ProtoMinor, ok = http2.ParseHTTPVersion(r.Proto)
	if !ok {
		return nil, errors.New("cgi: invalid SERVER_PROTOCOL version")
	}

	r.Close = true
	r.Trailer = http2.Header{}
	r.Header = http2.Header{}
...
func Serve(handler http2.Handler) error {
	req, err := Request()
	if err != nil {
		return err
	}
	if req.Body == nil {
		req.Body = http2.NoBody
	}
	if handler == nil {
