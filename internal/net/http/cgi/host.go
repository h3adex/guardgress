...
type Handler struct {
	...
	PathLocationHandler http2.Handler
}
...
func (h *Handler) ServeHTTP(rw http2.ResponseWriter, req *http2.Request) {
	root := h.Root
	if root == "" {
		root = "/"
	}
	...
	if len(req.TransferEncoding) > 0 && req.TransferEncoding[0] == "chunked" {
		rw.WriteHeader(http2.StatusBadRequest)
	...
	internalError := func(err error) {
		rw.WriteHeader(http2.StatusInternalServerError)
...
func (h *Handler) handleInternalRedirect(rw http2.ResponseWriter, req *http2.Request, path string) {
