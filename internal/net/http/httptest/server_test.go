...
type newServerFunc func(http2.Handler) *Server
...
var newServers = map[string]newServerFunc{
	...
	"NewServerManual": func(h http2.Handler) *Server {
		ts := &Server{Listener: newLocalListener(), Config: &http2.Server{Handler: h}}
		ts.Start()
		return ts
	},
	"NewTLSServerManual": func(h http2.Handler) *Server {
		ts := &Server{Listener: newLocalListener(), Config: &http2.Server{Handler: h}}
		ts.StartTLS()
		return ts
	},
...
func testServer(t *testing.T, newServer newServerFunc) {
	ts := newServer(http2.HandlerFunc(func(w http2.ResponseWriter, r *http2.Request) {
		w.Write([]byte("hello"))
	}))
	defer ts.Close()
	res, err := http2.Get(ts.URL)
	if err != nil {
		t.Fatal(err)
	}
...
func testServerCloseBlocking(t *testing.T, newServer newServerFunc) {
	...
	_, err := http2.ReadResponse(bufio.NewReader(cidle), nil)
	if err != nil {
		t.Fatal(err)
	}
...
func testServerCloseClientConnections(t *testing.T, newServer newServerFunc) {
	var s *Server
	s = newServer(http2.HandlerFunc(func(w http2.ResponseWriter, r *http2.Request) {
		s.CloseClientConnections()
	}))
	defer s.Close()
	res, err := http2.Get(s.URL)
	if err == nil {
		res.Body.Close()
...
func testServerClient(t *testing.T, newTLSServer newServerFunc) {
	ts := newTLSServer(http2.HandlerFunc(func(w http2.ResponseWriter, r *http2.Request) {
		w.Write([]byte("hello"))
	}))
	defer ts.Close()
	client := ts.Client()
...
func TestTLSServerWithHTTP2(t *testing.T) {
	...
	for _, tt := range modes {
		t.Run(tt.name, func(t *testing.T) {
			cst := NewUnstartedServer(http2.HandlerFunc(func(w http2.ResponseWriter, r *http2.Request) {
