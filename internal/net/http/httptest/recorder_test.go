...
func TestRecorder(t *testing.T) {
	...
	hasNotHeaders := func(keys ...string) checkFunc {
		return func(rec *ResponseRecorder) error {
			for _, k := range keys {
				v, ok := rec.Result().Header[http2.CanonicalHeaderKey(k)]
				if ok {
	...
	hasNotTrailers := func(keys ...string) checkFunc {
		return func(rec *ResponseRecorder) error {
			...
			for _, k := range keys {
				_, ok := trailers[http2.CanonicalHeaderKey(k)]
				if ok {
	...
	for _, tt := range [...]struct {
		name   string
		h      func(w http2.ResponseWriter, r *http2.Request)
		checks []checkFunc
	}{
		{
			"200 default",
		...
		},
		{
			"flush",
			func(w http2.ResponseWriter, r *http2.Request) {
				w.(http2.Flusher).Flush() // also sends a 200
				w.WriteHeader(201)
			},
		...
		t.Run(tt.name, func(t *testing.T) {
			r, _ := http2.NewRequest("GET", "http://foo.com/", nil)
			h := http2.HandlerFunc(tt.h)
			rec := NewRecorder()
			h.ServeHTTP(rec, r)
...
func TestRecorderPanicsOnNonXXXStatusCode(t *testing.T) {
	...
	for _, badCode := range badCodes {
		...
		t.Run(fmt.Sprintf("Code=%d", badCode), func(t *testing.T) {
			...
			handler := func(rw http2.ResponseWriter, _ *http2.Request) {
				rw.WriteHeader(badCode)
			}
			r, _ := http2.NewRequest("GET", "http://example.org/", nil)
			rw := NewRecorder()
			handler(rw, r)
		})
	}
