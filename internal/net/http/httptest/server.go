...
import (
	...
	"github.com/h3adex/guardgress/internal/net/http/internal/testcert"
	"log"
	"net"
	"os"
	"strings"
...
type Server struct {
	...
	closed bool
	conns  map[net.Conn]http2.ConnState // except terminal states
...
func NewServer(handler http2.Handler) *Server {
...
//
// The caller should call Close when finished, to shut it down.
func NewUnstartedServer(handler http2.Handler) *Server {
	return &Server{
		Listener: newLocalListener(),
		Config:   &http2.Server{Handler: handler},
	}
}
...
func (s *Server) Start() {
	...
	if s.client == nil {
		s.client = &http2.Client{Transport: &http2.Transport{}}
	}
...
func (s *Server) StartTLS() {
	...
	cert, err := tls2.X509KeyPair(testcert.LocalhostCert, testcert.LocalhostKey)
	if err != nil {
	...
	if len(s.TLS.Certificates) == 0 {
		s.TLS.Certificates = []tls2.Certificate{cert}
	}
	...
	s.client.Transport = &http2.Transport{
		TLSClientConfig: &tls2.Config{
			RootCAs: certpool,
		},
		ForceAttemptHTTP2: s.EnableHTTP2,
	}
	s.Listener = tls2.NewListener(s.Listener, s.TLS)
...
// The caller should call Close when finished, to shut it down.
func NewTLSServer(handler http2.Handler) *Server {
	ts := NewUnstartedServer(handler)
	ts.StartTLS()
	return ts
}
...
func (s *Server) Close() {
	...
	if !s.closed {
		...
		for c, st := range s.conns {
			...
			// in StateActive.
			if st == http2.StateIdle || st == http2.StateNew {
				s.closeConn(c)
			}
		}
	...
	if t, ok := http2.DefaultTransport.(closeIdleTransport); ok {
...
func (s *Server) Client() *http2.Client {
	return s.client
}
...
func (s *Server) wrap() {
	...
	s.Config.ConnState = func(c net.Conn, cs http2.ConnState) {
		s.mu.Lock()
		defer s.mu.Unlock()
		...
		case http2.StateNew:
			...
			if s.conns == nil {
				s.conns = make(map[net.Conn]http2.ConnState)
			}
			s.conns[c] = cs
			if s.closed {
		...
		case http2.StateActive:
			if oldState, ok := s.conns[c]; ok {
				if oldState != http2.StateNew && oldState != http2.StateIdle {
					panic("invalid state transition")
				}
				s.conns[c] = cs
			}
		case http2.StateIdle:
			if oldState, ok := s.conns[c]; ok {
				if oldState != http2.StateActive {
					panic("invalid state transition")
				}
				s.conns[c] = cs
			}
		...
		case http2.StateHijacked, http2.StateClosed:
			s.forgetConn(c)
		}
		if oldHook != nil {
			oldHook(c, cs)
