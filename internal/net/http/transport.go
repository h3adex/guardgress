...
import (
	...
	"github.com/h3adex/guardgress/internal/net/http/httptrace"
...
type Transport struct {
	...
	// automatically.
	TLSNextProto map[string]func(authority string, c *tls2.Conn) RoundTripper
...
func (t *Transport) Clone() *Transport {
	...
	if !t.tlsNextProtoWasNil {
		npm := map[string]func(authority string, c *tls2.Conn) RoundTripper{}
...
func (pconn *persistConn) addTLS(ctx context.Context, name string, trace *httptrace.ClientTrace) error {
	...
	tlsConn := tls2.Client(plainConn, cfg)
	...
	if err := <-errc; err != nil {
		...
		if trace != nil && trace.TLSHandshakeDone != nil {
			trace.TLSHandshakeDone(tls2.ConnectionState{}, err)
		}
		return err
	}
...
func (t *Transport) dialConn(ctx context.Context, cm connectMethod) (pconn *persistConn, err error) {
	...
	if cm.scheme() == "https" && t.hasCustomTLSDialer() {
		...
		if tc, ok := pconn.conn.(*tls2.Conn); ok {
	...
	if s := pconn.tlsState; s != nil && s.NegotiatedProtocolIsMutual && s.NegotiatedProtocol != "" {
		if next, ok := t.TLSNextProto[s.NegotiatedProtocol]; ok {
			alt := next(cm.targetAddr, pconn.conn.(*tls2.Conn))
...
// client or server.
func cloneTLSConfig(cfg *tls2.Config) *tls2.Config {
	if cfg == nil {
		return &tls2.Config{}
	}
	return cfg.Clone()
