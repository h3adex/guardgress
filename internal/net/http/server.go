...
func (c *conn) serve(ctx context.Context) {
	...
	if tlsConn, ok := c.rwc.(*tls2.Conn); ok {
		if d := c.server.ReadTimeout; d > 0 {
		...
		if err := tlsConn.HandshakeContext(ctx); err != nil {
			...
			if re, ok := err.(tls2.RecordHeaderError); ok && re.Conn != nil && tlsRecordHeaderLooksLikeHTTP(re.RecordHeader) {
		...
		c.tlsState = new(tls2.ConnectionState)
...
type Server struct {
	...
	// automatically.
	TLSNextProto map[string]func(*Server, *tls2.Conn, Handler)
...
func (srv *Server) ServeTLS(l net.Listener, certFile, keyFile string) error {
	...
	if !configHasCert || certFile != "" || keyFile != "" {
		var err error
		config.Certificates = make([]tls2.Certificate, 1)
		config.Certificates[0], err = tls2.LoadX509KeyPair(certFile, keyFile)
		if err != nil {
			return err
		}
	}

	tlsListener := tls2.NewListener(l, config)
