...
import (
	...
	"github.com/h3adex/guardgress/internal/net/http/cgi"
	"io"
	"net"
	"os"
	"strings"
...
func (r *response) Header() http2.Header {
	return r.header
}

func (r *response) Write(p []byte) (n int, err error) {
	if !r.wroteHeader {
		r.WriteHeader(http2.StatusOK)
	}
	if !r.wroteCGIHeader {
		r.writeCGIHeader(p)
	}
...
func (r *response) WriteHeader(code int) {
	...
	r.wroteHeader = true
	r.code = code
	if code == http2.StatusNotModified {
		// Must not have body.
	...
	if r.header.Get("Date") == "" {
		r.header.Set("Date", time.Now().UTC().Format(http2.TimeFormat))
	}
}
...
func (r *response) writeCGIHeader(p []byte) {
	...
	fmt.Fprintf(r.w, "Status: %d %s\r\n", r.code, http2.StatusText(r.code))
	if _, hasType := r.header["Content-Type"]; r.code != http2.StatusNotModified && !hasType {
		r.header.Set("Content-Type", http2.DetectContentType(p))
	}
	r.header.Write(r.w)
	r.w.WriteString("\r\n")
	r.w.Flush()
...
func newChild(rwc io.ReadWriteCloser, handler http2.Handler) *child {
	return &child{
		conn:     newConn(rwc),
		handler:  handler,
...
func (c *child) serveRequest(req *request, body io.ReadCloser) {
	...
	if err != nil {
		...
		r.WriteHeader(http2.StatusInternalServerError)
...
func Serve(l net.Listener, handler http2.Handler) error {
	if l == nil {
		var err error
...
func ProcessEnv(r *http2.Request) map[string]string {
