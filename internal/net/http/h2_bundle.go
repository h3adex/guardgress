...
// c is never closed.
func (p *http2clientConnPool) addConnIfNeeded(key string, t *http2Transport, c *tls2.Conn) (used bool, err error) {
	p.mu.Lock()
...
func (c *http2addConnCall) run(t *http2Transport, key string, tc *tls2.Conn) {
...
// connection.
func (t *http2Transport) dialTLSWithContext(ctx context.Context, network, addr string, cfg *tls2.Config) (*tls2.Conn, error) {
	dialer := &tls2.Dialer{
		Config: cfg,
	}
	...
	tlsCn := cn.(*tls2.Conn) // DialContext comment promises this will always succeed
	return tlsCn, nil
}
...
type http2connectionStater interface {
	ConnectionState() tls2.ConnectionState
}
...
func http2ConfigureServer(s *Server, conf *http2Server) error {
	...
	if s.TLSConfig == nil {
		s.TLSConfig = new(tls2.Config)
	} else if s.TLSConfig.CipherSuites != nil && s.TLSConfig.MinVersion < tls2.VersionTLS13 {
		...
		for _, cs := range s.TLSConfig.CipherSuites {
			switch cs {
			case tls2.TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256,
	...
	if s.TLSNextProto == nil {
		s.TLSNextProto = map[string]func(*Server, *tls2.Conn, Handler){}
	}
	protoHandler := func(hs *Server, c *tls2.Conn, h Handler) {
...
func (s *http2Server) ServeConn(c net.Conn, opts *http2ServeConnOpts) {
	...
	if tc, ok := c.(http2connectionStater); ok {
		sc.tlsState = new(tls2.ConnectionState)
		*sc.tlsState = tc.ConnectionState()
		...
		if sc.tlsState.Version < tls2.VersionTLS12 {
...
type http2serverConn struct {
	...
	tlsState         *tls2.ConnectionState       // shared by all handlers, like net/http
...
func (sc *http2serverConn) newWriterAndRequestNoBody(st *http2stream, rp http2requestParam) (*http2responseWriter, *Request, error) {
	...
	var tlsState *tls2.ConnectionState // nil if not scheme https
	if rp.scheme == "https" {
...
type http2Transport struct {
	...
	DialTLS func(network, addr string, cfg *tls2.Config) (net.Conn, error)
...
func http2configureTransports(t1 *Transport) (*http2Transport, error) {
	...
	if t1.TLSClientConfig == nil {
		t1.TLSClientConfig = new(tls2.Config)
	}
	...
	upgradeFn := func(authority string, c *tls2.Conn) RoundTripper {
	...
	if m := t1.TLSNextProto; len(m) == 0 {
		t1.TLSNextProto = map[string]func(string, *tls2.Conn) RoundTripper{
			"h2": upgradeFn,
		}
	} else {
		m["h2"] = upgradeFn
...
type http2ClientConn struct {
	...
	tlsState      *tls2.ConnectionState // nil only for specialized impls
...
func (t *http2Transport) newTLSConfig(host string) *tls2.Config {
	cfg := new(tls2.Config)
...
func (t *http2Transport) dialTLS(ctx context.Context) func(string, string, *tls2.Config) (net.Conn, error) {
	if t.DialTLS != nil {
		return t.DialTLS
	}
	return func(network, addr string, cfg *tls2.Config) (net.Conn, error) {
