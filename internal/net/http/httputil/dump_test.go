...
var dumpTests = []dumpTest{
	...
	{
		Req: mustNewRequest("POST", "http://example.com/foo", http2.NoBody),
...
func TestDumpRequest(t *testing.T) {
	...
	for i, tt := range dumpTests {
		...
		freshReq := func(ti dumpTest) *http2.Request {
			req := ti.Req
			if req == nil {
				req = ti.GetReq()
			}
...
func mustNewRequest(method, url string, body io.Reader) *http2.Request {
	req, err := http2.NewRequest(method, url, body)
	if err != nil {
...
func mustReadRequest(s string) *http2.Request {
	req, err := http2.ReadRequest(bufio.NewReader(strings.NewReader(s)))
	if err != nil {
		panic(err)
	}
	return req
...
func TestDumpRequestOutIssue38352(t *testing.T) {
	...
	for i := 0; i < 1000; i++ {
		...
		req, err := http2.NewRequestWithContext(ctx, http2.MethodPost, "http://example.com", r)
		if err != nil {
			t.Fatal(err)
		}
