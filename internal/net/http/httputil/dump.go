...
func drainBody(b io.ReadCloser) (r1, r2 io.ReadCloser, err error) {
	if b == nil || b == http2.NoBody {
		// No copying needed. Preserve the magic sentinel meaning of NoBody.
		return http2.NoBody, http2.NoBody, nil
	}
...
func outgoingLength(req *http2.Request) int64 {
	if req.Body == nil || req.Body == http2.NoBody {
		return 0
	}
...
// User-Agent.
func DumpRequestOut(req *http2.Request, body bool) ([]byte, error) {
	save := req.Body
	dummyBody := false
	if !body {
	...
	go func() {
		req, err := http2.ReadRequest(bufio.NewReader(pr))
		if err == nil {
...
func DumpRequest(req *http2.Request, body bool) ([]byte, error) {
	var err error
	save := req.Body
...
func DumpResponse(resp *http2.Response, body bool) ([]byte, error) {
	var b bytes.Buffer
	var err error
	save := resp.Body
